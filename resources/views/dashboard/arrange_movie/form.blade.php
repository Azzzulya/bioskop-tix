        ...
        <div class="col-md-8 offset-md-2 ">
          <form method="post" action="{{route($url, $theater->id ?? '')}}" enctype="multipart/form-data">
            ...
            <div class="form-group form-row my-4">
              ...
              <div class="col-5">
                <input type="number" placeholder="Cols" class="form-control @error('columns') {{ 'is-invalid' }} @enderror" name="columns" value="{{ old('columns') ?? $theater->columns ?? ''}}">
                @error('columns')
                <span class="text-danger">{{$message}}</span>
              @enderror
              </div>
           ...
           <div class="mb-2">
            <div class="form-group mb-0">
             ...
             <div class="form-check form-check-inline">
              <input type="radio" name="status" class="form-check-input" value="comming soon" id="comming soon" 
                @if ((old('status') ?? $theater->status ?? '' == 'comming soon'))
                    checked
                @endif>
            ...
            <div class="form-check form-check-inline">
              <input type="radio" name="status" class="form-check-input" value="in theater" id="in theater" 
                @if ((old('status') ?? $theater->status ?? '' == 'in theater'))
                    checked
                @endif>
              <label for="in theater" class="form-check-label">In theater</label>
            </div>
            <div class="form-check form-check-inline">
              <input type="radio" name="status" class="form-check-input" value="in theater" id="finish" 
